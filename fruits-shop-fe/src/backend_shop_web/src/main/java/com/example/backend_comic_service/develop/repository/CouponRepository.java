...
public interface CouponRepository extends JpaRepository<CouponEntity, Integer> {
    ...
    @Query(value = "update c set c.quantity = (c.quantity - 1) from orders od inner join coupon c on od.coupon_id = c.id \n" +
            " where od.id = ?1", nativeQuery = true)
    void updateQuantity(Integer orderId);
    @Query(value = "update coupon set [status] = 0 , is_delete = 1 where id = ?1", nativeQuery = true)
    void updateDeleteCoupon(Integer id);
