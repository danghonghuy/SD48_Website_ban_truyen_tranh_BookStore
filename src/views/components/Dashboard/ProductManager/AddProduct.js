...
const AddProduct = ({ fetchData, modelItem, textButton, isStyle }) => {
      ...
      if (data.code === 200) {
        ...
        if (data.data.images && data.data.images.length > 0) {
          data.data.images.map((item, index) => {
            return handleConvert(item, index);
          });
        }
      } else {
  ...
  const handleConvert = async (url, index) => {
    try {
      ...
      const fileName = url.imageUrl.split("/").pop(); // Extract the file name from the URL
      ...
      setFileList((previewItems) => [
        ...previewItems,
        {
          type: "curr",
          uid: index,
          id: url.id, // Required: unique id for each file
          name: fileName,
          status: "done",
  ...
  const handleRemove = (e) => {
    setFileListDelete([...fileListDelete, e]);
  };
  ...
  const onFinish = async (values) => {
    try {
      ...
      const files = [...fileList, ...fileListDelete];
      const fileCurr = files
        .filter((file) => file.type === "curr")
        .map((item) => ({
          imageUrl: item.url,
          id: item.id,
          isDeleted: item.status === "removed" ? 1 : 0,
        }));
      ...
      const fileUpload = files.filter((file) => file?.type !== "curr");
