...
const OrderDetail = () => {
  ...
  const handleProductSelected = (products, index) => {
    ...
    if (couponModel !== undefined && couponModel !== null) {
      if (couponModel.minValue <= totalPrice) {
        var discountNumber = 0;
        if (couponModel.type === 1) {
          discountNumber = (sum * couponModel.percentValue) / 100;
        } else {
  ...
  const fetchOrderDetail = async () => {
    ...
    if (!success || data.status == "Error") {
      ...
      modelTabs[0] = {
        ...modelTabs[0],
        products: data.data?.orderDetailModels.map((item) => ({
          ...item,
          isUpdate: true,
        })),
      ...
      let discountNumberCheck = 0;
      if (data.data?.couponModel?.type === 1) {
        discountNumberCheck =
          (sum * data.data?.couponModel?.percentValue) / 100;
      } else {
        discountNumberCheck = data.data?.couponModel?.couponAmount || 0;
      }
      form.setFieldsValue({
      ...
      setTabs(modelTabs);
      setTotalPrice(sum - discountNumberCheck + data.data?.feeDelivery);
      setFeeDelivery(data.data?.feeDelivery);
      if (
        data.data?.couponModel !== undefined &&
        data.data?.couponModel !== null
      ) {
        if (data.data?.couponModel.minValue <= sum) {
          var discountNumber = 0;
          if (data.data?.couponModel.type === 1) {
            discountNumber = (sum * data.data?.couponModel.percentValue) / 100;
          } else {
      ...
      if (data.data?.status === ORDER_STATUS.ORDER_STATUS_CUSTOMER_CANCEL) {
        ...
        setSteps(apiSteps);
      } else if (data.data?.deliveryModel?.id === 1) {
        const apiSteps = [
          {
            name: "Chờ xác nhận",
            createdDate: null,
  ...
  const onCreateOrder = async (modelProducts, tabIds) => {
    try {
      ...
      var objectModel = {
        id: orderModel.id,
        userId: userModel ? userModel.id : 1,
        price: totalPrice,
        paymentId: paymentId,
        feeDelivery: feeDelivery,
        deliveryType: deleveryId,
        description: null,
        status: ORDER_STATUS_OPTIONS.find((e) => e.value === orderModel.status)
          .code,
        stage: 1,
        type: 1,
        ...
        isDeliver: orderModel?.isDeliver,
        isChangeOrder: 1,
      };
  ...
  const columns = [
      ...
      render: (_, record, index) => {
        return (
          <InputNumber
            ...
            onChange={(e) => {
              if (e > record.stock) {
                toast.error("Số lượng vượt quá số lượng trong kho");
              }
              handleInputQuantity(index, e);
            }}
          />
        );
      },
    },
    {
      ...
      render: (text, record) => (
        <p style={{ fontSize: "13px", color: "black", fontWeight: "300" }}>
          {formatCurrencyVND(text * record.quantity)}
        </p>
      ),
    },
    {
      ...
      render: (_, record, index) => (
        <>
          {(!record?.isUpdate ||
            (record?.isUpdate &&
              tabs[tabs.findIndex((e) => e.id === activeTab)].products.length >
                1)) && (
  ...
  const handlePopupUpdateCustomer = async ({
    fullName,
    phoneNumber,
    email,
    provinceId,
    ...
    const fee = await fetchShippingFee(provinceId);
    setOrderModel({
      ...orderModel,
    ...
    setFeeDelivery(fee);
    const sum = orderModel?.orderDetailModels.reduce(
      (accumulator, currentItem) =>
        accumulator + currentItem.price * currentItem.quantity,
      0
    );
    let discountNumberCheck = 0;
    if (orderModel?.couponModel?.type === 1) {
      discountNumberCheck = (sum * orderModel?.couponModel?.percentValue) / 100;
    } else {
      discountNumberCheck = orderModel?.couponModel?.couponAmount || 0;
    }
    setTotalPrice(sum - discountNumberCheck + fee);
  };
  ...
  const handleOkAccept = (note) => {
    updateStatus(orderModel.id, ORDER_STATUS.ORDER_STATUS_ACCEPT, note);
    setIsModalAccept(false);
  };

  const handleOkAcceptDelivery = (note) => {
    updateStatus(orderModel.id, ORDER_STATUS.ORDER_STATUS_DELIVERY, note);
  ...
  const handleOkFinish = (note) => {
    updateStatus(orderModel.id, ORDER_STATUS.ORDER_STATUS_SUCCESS, note);
        ...
        <Card>
          <Row>
            ...
            <Col span={12}>
              ...
              {orderModel && orderModel?.deliveryModel && (
                <Col span={24}>
                  <Row align={"middle"} gutter={[16, 16]}>
                    <Col span={6}>
                      <p style={{ fontWeight: "500", marginBottom: 0 }}>
              ...
              <Col span={24}>
                <Row align={"middle"} gutter={[16, 16]}>
                  ...
                  <Col span={18} style={{ textAlign: "right" }}>
                    ...
                    >
                      {formatCurrencyVND(orderModel ? totalPrice : 0)}
                    </p>
                  </Col>
                </Row>
              </Col>
